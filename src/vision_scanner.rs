...
use crate::{
    ...
    preprocess,
    robot_service::{FaceScanner, WrongDetectionCountError},
    tpu::CviTpuDetector,
};
...
impl FaceScanner for VisionScanner {
    fn begin_scan(&mut self, _revision: u32) -> Result<()> {
        std::fs::create_dir_all(&self.record_dir)?;
        let directory = create_next_record_dir(&self.record_dir)?;
...
fn create_next_record_dir(base: &Path) -> Result<PathBuf> {
    let mut maximum = 0_u64;
    for entry in std::fs::read_dir(base)
        .with_context(|| format!("could not list scan record directory {}", base.display()))?
    {
        let entry = entry.with_context(|| {
            format!(
                "could not read an entry in scan record directory {}",
                base.display()
            )
        })?;
        if let Some(index) = scan_record_index(&entry.file_name()) {
    ...
    let mut index = maximum
        .checked_add(1)
        .context("scan record index is exhausted")?;
    loop {
        let directory = base.join(format!("scan-{index:06}"));
...
fn scan_record_index(name: &std::ffi::OsStr) -> Option<u64> {
    let digits = name.to_str()?.strip_prefix("scan-")?;
...
fn recognized_face(detections: &[Detection]) -> Result<link::RecognizedFace> {
    if detections.len() != link::STICKERS_PER_FACE {
        return Err(WrongDetectionCountError {
