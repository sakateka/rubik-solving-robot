//! YOLOv8 model definition, vendored from the candle examples
//! (candle-examples/examples/yolo-v8, huggingface/candle, MIT/Apache-2.0).
//!
//! Adaptations:
//!  - imports point to candle_core/candle_nn instead of the `candle` meta-crate
//!  - tracing spans removed (fewer dependencies for the embedded target)
//!  - pose-estimation head removed (we only need object detection)
//!
//! The weight layout matches the ultralytics checkpoint structure
//! (net/fpn/head prefixes), so converted .safetensors files load directly.
...
impl Multiples {
    /// YOLOv8n — the smallest variant, the only one we use (256 MB RAM target).
    pub fn n() -> Self {
        Self {
            depth: 0.33,
            width: 0.25,
...
/// ConvBlock = Conv2d + BatchNorm + SiLU.
/// SiLU = Sigmoid Linear Unit (a.k.a. swish): x * sigmoid(x), the activation
/// used throughout YOLOv8.
#[derive(Debug)]
struct ConvBlock {
    conv: Conv2d,
}
...
impl ConvBlock {
    fn load(
        ...
        // The batch-norm is folded into the conv weights at load time:
        // one less op per conv at inference, less memory traffic.
...
/// C2f = the main feature-aggregation block of YOLOv8: a CSP-style block
/// (CSP = Cross Stage Partial connections) with 2 convolutions ("f" = the
/// faster variant). It splits the features in two, runs one half through a
/// chain of bottlenecks, then concatenates everything back.
#[derive(Debug)]
struct C2f {
    cv1: ConvBlock,
    cv2: ConvBlock,
...
/// SPPF = Spatial Pyramid Pooling, Fast variant. Chains several max-pools
/// to grab context at several scales cheaply; sits at the end of the
/// backbone.
#[derive(Debug)]
struct Sppf {
    cv1: ConvBlock,
...
/// DFL = Distribution Focal Loss module. The box edges are not predicted
/// directly; instead each edge is a softmax distribution over `num_classes`
/// bins (16 for YOLOv8). The conv computes its expectation, which
/// dist2bbox() then turns into box coordinates.
#[derive(Debug)]
struct Dfl {
    conv: Conv2d,
    num_classes: usize,
...
/// The backbone: progressively downsamples the image and extracts feature
/// maps at 3 scales (strides 8, 16, 32) — those feed the neck.
#[derive(Debug)]
struct DarkNet {
    b1_0: ConvBlock,
    b1_1: ConvBlock,
...
/// The neck, FPN-style (FPN = Feature Pyramid Network): merges the 3 scales
/// coming from the backbone, so that fine detail (small objects) and coarse
/// context (large objects) both reach the detection head.
#[derive(Debug)]
struct YoloV8Neck {
    up: Upsample,
    n1: C2f,
...
impl DetectionHead {
    ...
    /// Returns the raw prediction tensor [1, 4 + num_classes, anchors]:
    /// box centers/sizes in input-image pixels and per-class probabilities
...
impl Module for YoloV8 {
    /// Input: [1, 3, H, W] normalized to 0..1.
    /// Output: [1, 4 + num_classes, anchors] — see DetectionHead::forward.
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
