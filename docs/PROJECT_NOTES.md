...
ходом той же грани и канонизирует всю пару перед переходом к соседней оси.
Последовательный блок `F/B` использует один разворот всего кубика. Старый
алгоритм сохранён как `baseline_held_steps` только для сравнения и тестов.
