...
#### Stateful optimization механических движений

Проверенный executor пока консервативен: каждый логический Singmaster move
самостоятельно возвращает все rails в `NearGrip`, а grippers — в
`FramePerpendicular`. Это даёт простую границу безопасности, но создаёт лишние
перехваты. Особенно дорого стоят последовательности противоположных граней
(`L/R`, `U/D`) и блоки `F/B`, для которых весь куб сейчас разворачивается и
возвращается к Front после каждого отдельного хода.

Следующий planner должен переносить commanded mechanical state между ходами.
Возврат в `CanonicalGrip` требуется не после каждого move, а перед
несовместимым переходом, scan/open и в конце всей последовательности. Первый
безопасный набор оптимизаций:

- оставлять повернувший gripper в parallel endpoint до необходимости
  перехвата;
- выполнять ходы противоположных граней в одном holding mode и возвращать оба
  gripper в perpendicular одним парным переходом;
- держать один разворот всего кубика на протяжении последовательного блока
...
optimized action traces, число servo commands и ожидаемое время по timing из
`StandCalibration`. Тесты обязаны симулировать rails/grippers после каждого
шага, запрещать collision соседних parallel grippers, подтверждать непрерывное
удержание кубика и финальный `CanonicalGrip`. Старый planner остаётся baseline
до завершения сравнительных тестов.

Stateful planner реализован в `src/move_planner.rs` и подключён к solver
`Execute` и manual `ExecuteMoves`. Он переносит parallel endpoint только внутри
одной пары противоположных осей, сбрасывает конкретный gripper перед повторным
ходом той же грани и канонизирует всю пару перед переходом к соседней оси. Во
фрейме `RotatedFB` ходы U/D выполняются без возврата canonical front, поэтому
F/B-блоки, разделённые только U/D, используют одну общую пару переориентаций.
Старый алгоритм сохранён как `baseline_held_steps` только для сравнения и
тестов.

Dry-run не открывает I²C и не требует `--confirm-stand-motion`:

```sh
cargo run --features pca9685 --bin rubik-move-plan -- "F B R2 U' L"
```
...
progress и диагностическая последовательность остаются буквально
сопоставимыми. Такое сокращение можно добавить отдельным слоем вместе с
явным mapping исходных move indices на оптимизированные операции.
